...
module.exports = {
  ...
  dataTable: function(req, res) {
    var query = coreQueryBuilderService.buildQuery(req);
    loanApplicationService.dataTable(query, function(
      err,
      loanApplications
    ) {
      if (err) return res.status(400).send(err);
  ...
  add: function(req, res) {
    loanApplicationService.add(req.body, function(
      err,
      loanApplication
    ) {
      if (err) return res.status(400).send(err);
  ...
  update: function(req, res) {
    loanApplicationService.update(req.body, function(
      err,
      loanApplication
    ) {
      if (err) return res.status(400).send(err);
  ...
  remove: function(req, res) {
    var query = coreQueryBuilderService.buildQuery(req);
    loanApplicationService.remove(query, function(
      err,
      loanApplication
    ) {
      if (err) return res.status(400).send(err);
